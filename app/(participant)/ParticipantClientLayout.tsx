...
export default function ParticipantClientLayout({
  children,
}: {
  ...
  const navItems = [
    { href: "/participant/dashboard", icon: Home, label: t.common.home },
    { href: "/participant/events", icon: Calendar, label: t.common.discover },
    { href: "/participant/registrations", icon: Ticket, label: t.common.myEvents },
    { href: "/participant/profile", icon: User, label: t.common.profile },
  ];
      ...
      <header className="flex items-center justify-between p-4 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-lg border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10 shadow-sm dark:shadow-zinc-900/50 transition-colors duration-300">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#6B5A4E] dark:text-zinc-400">{t.nav.participantPortal}</p>
          </div>
        </div>
          ...
          return (
            <Link 
              ...
              className={`flex flex-col items-center space-y-1 px-4 py-2 rounded-xl transition-all ${
                isActive 
                  ? 'text-[#E89D71] bg-[#FEF3EB] dark:bg-[#E89D71]/20' 
                  : 'text-[#6B5A4E] dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800'
              }`}
            >
              <Icon className={`w-5 h-5 ${isActive ? 'text-[#E89D71]' : 'text-[#6B5A4E] dark:text-zinc-400'}`} />
              <span className={`text-[10px] font-bold ${isActive ? 'text-[#E89D71]' : 'text-[#6B5A4E] dark:text-zinc-400'}`}>
                {item.label}
              </span>
            </Link>
          );
