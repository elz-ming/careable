...
export default function EventDetailPage() {
  ...
  const handleConfirmRegistration = async () => {
    ...
    try {
      // If we are showing name prompt, we pass the fullName to update profile
      const result = await registerForEvent(id as string, showNamePrompt ? fullName : undefined)
      if (result.success) {
        setIsRegistered(true)
        setShowNamePrompt(false)
        // Optionally refresh profile to clear is_first_time
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20 p-4 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[#6B5A4E]">Careable Event Details</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <Card className="rounded-3xl border-zinc-100 shadow-sm p-6 bg-white space-y-6">
            ...
            <div className="pt-6 border-t border-zinc-50">
              {isRegistered ? (
                <div className="bg-green-50 p-4 rounded-xl flex items-center gap-3 text-green-700">
                ...
                >
                  {isRegistering ? <Loader2 className="w-5 h-5 animate-spin" /> : "Register for Event"}
                </Button>
              )}
            </div>
          </Card>
          ...
          {/* Registration Info */}
          <Card className="rounded-3xl border-dashed border-2 border-zinc-100 p-6 bg-zinc-50/50">
            <h4 className="font-bold text-[#2D1E17] mb-2 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-[#E89D71]" />
              Quick Info
            </h4>
            <ul className="text-xs text-zinc-500 space-y-2 list-disc pl-4">
              <li>Free for all eligible participants.</li>
          ...
          <Card className="w-full max-w-md rounded-3xl shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex gap-3">
                ...
                >
                  {isRegistering ? <Loader2 className="w-5 h-5 animate-spin" /> : "Complete & Register"}
                </Button>
              </div>
            </CardContent>
          </Card>
